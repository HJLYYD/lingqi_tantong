...
#define SERVO_PIN_H 40   // 左右
#define SERVO_PIN_V 41   // 上下
#define SERVO_COUNT 2
#define SERVO_CCW_US 1000   // 逆时针 / 上 / 左
#define SERVO_STOP_US 1500  // 停止
#define SERVO_CW_US 2000    // 顺时针 / 下 / 右
...
static const ledc_channel_t SERVO_CHANNELS[SERVO_COUNT] = { LEDC_CHANNEL_3, LEDC_CHANNEL_4 };

enum ServoDir { SERVO_DIR_STOP = 0, SERVO_DIR_CCW = -1, SERVO_DIR_CW = 1 };
ServoDir g_servo_dir[SERVO_COUNT] = { SERVO_DIR_STOP, SERVO_DIR_STOP };
...
bool handleServo(int servo_id, int angle, char* resp, size_t resp_size, size_t* resp_len);
void initServo();
void setServoPulseUs(int servo_id, uint32_t pulse_us);
void setServoDirection(int servo_id, ServoDir dir);
void stopServo(int servo_id);
...
void setup() {
  ...
  Serial.print("CoAP 舵机:   coap://"); Serial.print(ip); Serial.println("/servo   (PUT servo:0=GPIO40左右, servo:1=GPIO41上下)");

  // 先初始化舵机 PWM，再初始化摄像头，避免 LEDC 重配置影响 XCLK
  initServo();
  stopServo(0);
  stopServo(1);
  ...
  stopServo(0);
  stopServo(1);
  Serial.printf("连续旋转舵机已就绪 (GPIO%d=左右, GPIO%d=上下)\n",
                SERVO_PIN_H, SERVO_PIN_V);
...
void initServo() {
  ...
  for (int i = 0; i < SERVO_COUNT; i++) {
    ...
    ccfg.gpio_num = SERVO_PINS[i];
    ccfg.speed_mode = SERVO_LEDC_MODE;
    ccfg.channel = SERVO_CHANNELS[i];
...
void setServoPulseUs(int servo_id, uint32_t pulse_us) {
  ...
  ledc_set_duty(SERVO_LEDC_MODE, SERVO_CHANNELS[servo_id], duty);
  ledc_update_duty(SERVO_LEDC_MODE, SERVO_CHANNELS[servo_id]);
}

void setServoDirection(int servo_id, ServoDir dir) {
  if (servo_id < 0 || servo_id >= SERVO_COUNT) return;

  g_servo_dir[servo_id] = dir;
  uint32_t pulse_us = SERVO_STOP_US;
  if (dir == SERVO_DIR_CCW) pulse_us = SERVO_CCW_US;
  else if (dir == SERVO_DIR_CW) pulse_us = SERVO_CW_US;
  setServoPulseUs(servo_id, pulse_us);
}

void stopServo(int servo_id) {
  setServoDirection(servo_id, SERVO_DIR_STOP);
}

bool handleServo(int servo_id, int angle, char* resp, size_t resp_size, size_t* resp_len) {
  if (servo_id < 0 || servo_id >= SERVO_COUNT) return false;

  // CoAP: 0=逆时针/上/左, 90=停止, 180=顺时针/下/右
  if (angle == 0) setServoDirection(servo_id, SERVO_DIR_CCW);
  else if (angle == 180) setServoDirection(servo_id, SERVO_DIR_CW);
  else setServoDirection(servo_id, SERVO_DIR_STOP);

  const char* dir_str = "stop";
  if (g_servo_dir[servo_id] == SERVO_DIR_CCW) dir_str = (servo_id == 0) ? "left" : "up";
  else if (g_servo_dir[servo_id] == SERVO_DIR_CW) dir_str = (servo_id == 0) ? "right" : "down";

  int n = snprintf(resp, resp_size,
    "{\"servo\":%d,\"angle\":%d,\"dir\":\"%s\",\"ok\":true}", servo_id, angle, dir_str);
