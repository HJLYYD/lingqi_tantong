...
4. [模块详解](#4-模块详解)
   ...
   - 4.3 [YOLOv8n-Pose 姿态估计器](#43-yolov8n-pose-姿态估计器)
...
| 功能      | 描述                                              | 使用的模型/算法                             |
| ------- | ----------------------------------------------- | ------------------------------------ |
| 人员检测    | YOLOv8n-Pose 统一模型同时完成人员检测和 17 点姿态估计             | YOLOv8n-Pose (INT8 量化)               |
...
推理管道负责多模型的协调调度，实现了自适应级联状态机和关键点验证过滤。当前采用统一模型模式 (YOLOv8n-Pose 同时完成检测和姿态估计)。
...
模型注册表:
  ...
  │ 模型名称                 │ 配置键           │ 类型         │
  ├─────────────────────────────────────────────────────────────┤
  │ yolov8n_pose            │ pose.model_path   │ YOLOv8 Pose  │
...
YOLOv8n-Pose 是项目的核心模型，采用 Ultralytics YOLOv8n-Pose 统一模型，同时输出人员检测框和 17 个 COCO 关键点。支持标准 ONNX 和 xquant INT8 量化分割模型两种格式。
...
| 预期功能          | 实现状态 | 实现方法                                 |
| ------------- | ---- | ------------------------------------ |
| 统一检测+姿态估计     | 已实现  | 单个 YOLOv8n-Pose 模型同时输出 bbox + 17 关键点 |
...
2. **YOLO11 Pose 使用更小的 n 模型变体**: 已有 YOLOv8n-Pose (nano, 2.9M)，可评估 s/m 模型在 K1 上的性能
...
const char* config_get_string(cm, "pose.model_path", "models/yolov8n-pose.q.onnx");
```
...
模型名称                → 实际路径 (由配置指定)
──────────────────────────────────────────────────────────
yolov8n_pose           → models/.../yolov8n-pose.q.onnx
...
状态影响推理行为:
  SEARCHING:  YOLOv8n-Pose 全分辨率, 人脸每 10 帧
  TRACKING:   YOLOv8n-Pose 全分辨率, 人脸每 30 帧
  VALIDATING: YOLOv8n-Pose 全分辨率, 人脸每 10 帧 (1帧后回 TRACKING)
...
| -- | --------------------- | ---------------------------------- | ------- | ------------- | --------------- |
| 1  | YOLOv8n-Pose          | yolov8n-pose.q.onnx                | INT8 量化 | 480×480       | 人员检测 + 17 点姿态估计 |
...
│ 模型                     │ EP                   │ 推理核心     │
├──────────────────────────┼──────────────────────┼─────────────┤
│ YOLOv8n-Pose (INT8)      │ SpacemiT EP          │ Cluster0 IME│
...
| ---- | ----------------------------------- | ------------------------------- | ----------------------------------------- |
| 姿态估计 | YOLOv8n-Pose (INT8) + DFL peakiness | YOLOv8-Pose (FP16) + 标准置信度      | INT8 量化 + DFL 方案是 K1 特化，FP16 方案精度更高但无硬件加速 |
